...
async fn healthchecks(req: HttpRequest) -> Result<HttpResponse, CustomError> {
    ...
    println!("[{:?} INFO ] - squirrel-tactix - /health GET (health check) request - from {:?} - {:?}", readi, peer, &requ);
    let users = Users::find(1)?;
    let readb: DateTime<Utc> = Utc::now();
    match users {
      users => println!("[{:?} INFO ] - squirrel-tactix - /health GET (health check) response OK - {:?}", readb, requ),
    };
    let hresp = "RECV";
